...
for i in range(wave_len_tr.shape[0]):
	mat_tr_sgs[i,0] = (wave_len_tr[i] * (1e-7))
...
energ = 5.21*1e-7 ##vega erg/(c*cm^2) in J
norm = area/energ
print 'sgs:'
print area
print norm
...
for i in range(mat_sgs.shape[0]):
	mat_sgs[i,1] = ((mat_sgs[i,1]/norm)*4.36e-3*mat_sgs[i,0]*5.03e15)
	mat_sgs[i,1] = mat_sgs[i,1]*4835107.4

for i in range(mat_res.shape[0]):
	mat_res[i,1] = ((mat_res[i,1]/norm)*4.36e-3*mat_res[i,0]*5.03e15)
	mat_res[i,1] = mat_res[i,1]*4835107.4

np.savetxt('our_star_phot.txt',mat_sgs,delimiter=' ',fmt='%e')
np.savetxt('our_star_photJ.txt',mat_res,delimiter=' ',fmt='%e')
